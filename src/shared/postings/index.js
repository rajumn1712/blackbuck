  ...
  renderCourseCard = (post) => {
    ...
    return <>{post.CourseType === "Live Session" ? <div className="livecourse-card mx-16">
      <div className="p-relative">
        <img onClick={() => { window.open(process.env.PUBLIC_URL+`/course/${post.CourseId}`, "_blank") }} width="100%" height="240" src={liveIcon[post.UrlType]} className="zoom-img" />
        <div className="live-btn-hover d-flex align-items-center">
          <a className="f-24 semibold" onClick={() => { window.open(process.env.PUBLIC_URL+`/course/${post.CourseId}`, "_blank") }}>Join Live Session</a>
        </div>
      </div>
